<script lang="ts">
	...
	// Updated filtering logic to handle multiple platforms per tool
...
<div class="space-y-6 p-6">
	<div class="mx-auto mt-10 mb-30 space-y-10 sm:mt-5 md:max-w-2xl">
		<div class="text-center">
			<h1 class="text-3xl leading-tight font-extrabold sm:text-6xl">
		...
		<div class="flex items-center gap-6">
			...
			<button
				...
				class="z-20 cursor-pointer rounded-[16px] border border-[#4E71FF] bg-[#3A59D1] p-3 px-6 text-center delay-100 hover:bg-[#362FD9]"
			>
				Filters
			</button>
		</div>
			...
			>
				...
				<div class="grid grid-cols-2 gap-3">
					{#each categories as category}
						<button
							class="cursor-pointer rounded-[10px] border p-3 {selectedCategories.includes(
								category.slug
							)
