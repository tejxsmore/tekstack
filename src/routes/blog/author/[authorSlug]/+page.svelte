<script lang="ts">
	...
	const totalPages = $derived(Math.ceil(author.post.length / postsPerPage));
	const startIndex = $derived((currentPage - 1) * postsPerPage);
	const endIndex = $derived(startIndex + postsPerPage);
	const paginatedPosts = $derived(author.post.slice(startIndex, endIndex));
...
<div class="space-y-6 p-6">
	...
	<!-- Pagination Info -->
	{#if author.post.length > 0}
		<div class="mb-6 flex items-center justify-between">
			<p class="text-sm text-gray-400">
				Showing {startIndex + 1}-{Math.min(endIndex, author.post.length)} of {author.post.length}
				blogs
			</p>
			{#if totalPages > 1}
				<p class="text-sm text-gray-400">
	...
	<div class="grid-cols mb-8 grid gap-6 md:grid-cols-2 xl:grid-cols-3">
		...
		{:else}
			<div class="col-span-full text-center py-12">
				...
				<p class="mt-1.5 text-sm text-gray-500">This author hasn't published any blogs yet</p>
			</div>
		{/each}
	</div>
